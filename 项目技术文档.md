...
| **样式** | Tailwind CSS | 原子化 CSS，支持深色模式 |
| **认证** | JWT | 无状态认证 |
...
│   ├── index.html
│   ├── vite.config.ts
│   └── tailwind.config.js         # 含自定义断点 xs:480px，darkMode: 'class'
│
...
| 协议 | `otpauth://` | 固定前缀 |
| 类型 | `totp` | 基于时间的一次性密码 |
| issuer | `Google` | 发行方（路径中冒号前的部分） |
| name | `user@gmail.com` | 账号名称（路径中冒号后的部分） |
| secret | `JBSWY3DPEHPK3PXP` | Base32 编码的密钥 |
| issuer | `Google` | 发行方（参数，与路径中的一致） |
| digits | `6` | 验证码位数 |
| period | `30` | 刷新周期（秒） |
...
**存储**：用户偏好保存在 `localStorage.theme`

### 9.3 UI 设计

- 右下角悬浮圆形按钮（48x48px），适配移动端触控
- 点击展开选择菜单，当前模式高亮显示
- 图标随当前模式变化：电脑（自动）、太阳（浅色）、月亮（深色）
- 所有页面、弹窗、卡片均已适配深色主题
